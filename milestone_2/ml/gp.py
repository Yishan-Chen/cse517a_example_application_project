...
def compute_se_kernel_matrix(Xs, Ys, lsquared):
    if Xs is Ys: dist = sp.squareform(sp.pdist(Xs, "sqeuclidean"))
    else: dist = sp.cdist(Xs, Ys, "sqeuclidean")
...
if __name__ == "__main__":
    ...
    ntrain = 1000      # number of training samples used
    ntest =  600       # number of testing samples used

    deskew = True       # deskew input images or not (by default: yes)
    normalize = True    # normalize input vectors or not (by default: yes)

    lsquared = 1.0     # l^2 used in SE kernel computation
    ...
    X_train, T_train = preprocessing.get_training_data(ntrain, normalize=normalize, deskew=deskew)
    X_test, T_test = preprocessing.get_testing_data(ntest, normalize=normalize, deskew=deskew)
